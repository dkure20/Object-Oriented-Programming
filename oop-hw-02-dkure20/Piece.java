...
public class Piece {
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] newT = new TPoint[body.length];
		for(int i=0; i<body.length; i++){
			newT[i] = new TPoint(height - body[i].y- 1, body[i].x);
		}
		Piece newBody = new Piece(newT);
		return newBody;
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		return Arrays.equals(other.body,body);
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece prePiece = root;
		Piece current = root.computeNextRotation();
		root.next = current;
		while(true){
